...
def get_average_sentence_length(text):
    ...
    punct = text.replace("?", ".")
    better_punct = punct.replace("?!", ".")
    best_punct = better_punct.replace("!", ".")
    sentences_in_text = best_punct.split(".")
    sentence_len = []
...
class TextSample:
    def __init__(self, text, author):
        ...
        self.ngram_frequency = build_frequency_table(ngram_creator(self.prepare_text))
    def __repr__(self):
        return "{name}'s letter had an average sentence length of {length}".format(name=self.author, length=self.average_sentence_length)
...
def prepare_text(text):
    new_text = text.lower().replace('.', ' ').replace('!', ' ').replace('?', ' ').replace(',', ' ').strip().split()
    return new_text
...
def ngram_creator(text_list):
    ...
    for n in range(len(text_list) - 1):
        ngrams.append(text_list[n] + " " + text_list[n + 1])
    return ngrams
...
def find_text_similarity(txt1, txt2):
    ...
    word_count_similarity = frequency_comparison(txt1.word_count_frequency, txt2.word_count_frequency)
    ngram_similarity = frequency_comparison(txt1.ngram_frequency, txt2.ngram_frequency)
...
print(str(lily_sample.author) + "'s similarity to the murderer note: " + str(find_text_similarity(lily_sample, murderer_sample)))
print(str(myrtle_sample.author) + "'s similarity to the murderer note: " + str(find_text_similarity(myrtle_sample, murderer_sample)))
print(str(gregg_sample.author) + "'s similarity to the murderer note: " + str(find_text_similarity(gregg_sample, murderer_sample)))
...
print("Gregg T Fishy murdered Jay Stacksby")
